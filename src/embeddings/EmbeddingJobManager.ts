...
export class EmbeddingJobManager {
  ...
  constructor(
    ...
    if (cacheOptions) {
      ...
      this.cacheOptions = {
        ...
        ttl: cacheOptions.ttl || (cacheOptions.ttlHours ? Math.round(cacheOptions.ttlHours * 60 * 60 * 1000) : 3600000)
      };
    }
    ...
    for (const job of jobs) {
      ...
      try {
        ...
        this.logger.debug('Retrieved entity for embedding', { 
          ...
          observationsLength: entity.observations && Array.isArray(entity.observations) ? entity.observations.length : 'n/a'
        });
   ...
   */
  private _updateJobStatus(jobId: string, status: JobStatus, attempts?: number, error?: string): any {
    let sql = `
      UPDATE embedding_jobs
      SET status = ?
    `;
