  ...
  try {
    switch (name) {
      ...
      case "get_relation":
        ...
        if (!relation) {
          return { content: [{ type: "text", text: `Relation not found: ${args.from} -> ${args.relationType} -> ${args.to}` }] };
        }
      ...
      case "get_entity_history":
        try {
          ...
          return { content: [{ type: "text", text: `Error retrieving entity history: ${error.message}` }] };
        }
      ...
      case "get_relation_history":
        try {
          ...
          return { content: [{ type: "text", text: `Error retrieving relation history: ${error.message}` }] };
        }
      ...
      case "get_graph_at_time":
        try {
          ...
          return { content: [{ type: "text", text: `Error retrieving graph at time: ${error.message}` }] };
        }
      ...
      case "get_decayed_graph":
        try {
          ...
          return { content: [{ type: "text", text: `Error retrieving decayed graph: ${error.message}` }] };
        }
      ...
      case "force_generate_embedding":
        ...
        try {
          ...
          if (allEntities && allEntities.entities && allEntities.entities.length > 0) {
            ...
            if (!entity) {
              process.stderr.write(`[DEBUG] Entity not found in list. Available entities: ${JSON.stringify(allEntities.entities.map((e: any) => ({ name: e.name, id: e.id })))}\n`);
            }
          } else {
          ...
          if (!entity && isUUID && knowledgeGraphManager.storageProvider && 
          ...
          return { content: [{ type: "text", text: `Failed to generate embedding: ${error.message}` }] };
        }
      ...
      case "semantic_search":
        try {
          ...
          return { content: [{ type: "text", text: `Error performing semantic search: ${error.message}` }] };
        }
      ...
      case "get_entity_embedding":
        try {
          ...
          if (knowledgeGraphManager.storageProvider && 
            ...
            const embedding = await (knowledgeGraphManager.storageProvider as any).getEntityEmbedding(args.entity_name);
            
            if (!embedding) {
              return { content: [{ type: "text", text: `No embedding found for entity: ${args.entity_name}` }] };
            }
            ...
            return { content: [{ type: "text", text: `Embedding retrieval not supported by this storage provider` }] };
          }
        } catch (error: any) {
          return { content: [{ type: "text", text: `Error retrieving entity embedding: ${error.message}` }] };
        }
        ...
        try {
          ...
          if (hasEmbeddingJobManager && knowledgeGraphManager.embeddingJobManager.embeddingService) {
            try {
              ...
              process.stderr.write(`[ERROR] Error getting embedding service info: ${error.message}\n`);
            }
          }
          ...
          if (storageProvider && storageProvider.embeddingService) {
            try {
              ...
              process.stderr.write(`[ERROR] Error getting embedding provider info: ${error.message}\n`);
            }
          }
      ...
      case "diagnose_vector_search":
        if (typeof (knowledgeGraphManager.storageProvider as any).diagnoseVectorSearch === 'function') {
          return { content: [{ type: "text", text: JSON.stringify(await (knowledgeGraphManager.storageProvider as any).diagnoseVectorSearch()) }] };
        } else {
          return {
            content: [{ 
              type: "text", 
