 ...
 * 
 * Run with: npx ts-node scripts/addEntityTypeLabels.ts
 */

import { Neo4jConnectionManager } from '../src/storage/neo4j/Neo4jConnectionManager';

// Function to sanitize entity type for use as a Neo4j label
function sanitizeLabel(entityType: string): string {
  ...
  try {
    ...
    for (const entity of entities) {
      ...
      try {
        ...
        console.error(`Error adding label to entity '${entity.name}':`, error);
        skipped++;
      }
    }
